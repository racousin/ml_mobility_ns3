...
def haversine_distance(lat1, lon1, lat2, lon2):
    ...
    This computes real distance on the Earth's surface (sphere), which is
    more accurate than Euclidean approximation for GPS coordinates.
...
class TrajectoryPreprocessor:
    ...
    def __init__(self, config):
        ...
        # Maximum physically realistic speeds per transport mode (km/h)
        # Used for outlier detection: if computed speed exceeds this,
        # the GPS point is considered erroneous
        self.max_speeds = {
            'WALKING': 15,
            'BIKE': 60,
            'PUBLIC_TRANSPORT': 150,
            'CAR': 200,
            'MIXED': 200  # Multimodal trips use the most permissive threshold
        }
        ...
        for user_id in tqdm(user_ids, desc="Processing users"):
            ...
            for _, trip in user_trips.iterrows():
                ...
                if len(trip_gps) > 0:
                    ...
                    # === DATA CLEANING (Professor's requirement) ===
                    # Step 1: Remove privacy-blurred points (H3 centroid snapping)
                    gps_array = self._remove_privacy_points(gps_array)

                    # Step 2: Replace speed outlier coordinates (NOT remove!)
                    # This ensures removing outliers does not create new outliers
                    if len(gps_array) >= 2:
                        gps_array = self._clean_speed_outliers(
                            gps_array, trip['category']
                        )
    ...
    def _remove_privacy_points(self, gps_points):
        """
        Remove privacy-blurred GPS points at the beginning and end of trips.

        Background (from NetMob25 paper, Section 3 - Anonymization):
            "The first and last GPS points of each trip, along with any
            subsequent points falling within the same spatial cell, are mapped
            to the centroid of the corresponding H3 hexagonal grid cell at
            resolution 10. This process hides approximately the first and
            last 50 to 100 meters of each trip."

        Detection method:
            Privacy points have IDENTICAL lat/lon coordinates (snapped to
            H3 centroid). We scan from the start and end of the trip for
            sequences of >= 2 consecutive points with exact same coordinates.

        Safeguards:
            - Requires >= 2 consecutive identical points (1 point could be
              a legitimate stop, e.g. waiting at a red light)
            - Never removes more than 30% of total points
        """
        if len(gps_points) < 4:  # Need at least 4 points to trim
            return gps_points

        n = len(gps_points)
        max_trim = int(n * 0.3)  # Safety: never trim more than 30%

        # --- Scan from the START ---
        start_trim = 0
        if n >= 2:
            ref_lat = float(gps_points[0, 1])
            ref_lon = float(gps_points[0, 2])
            count = 1
            for i in range(1, n):
                if (float(gps_points[i, 1]) == ref_lat and
                        float(gps_points[i, 2]) == ref_lon):
                    count += 1
                else:
                    break
            if count >= 2:  # At least 2 identical points = privacy segment
                start_trim = min(count, max_trim)

        # --- Scan from the END ---
        end_trim = 0
        if n >= 2:
            ref_lat = float(gps_points[-1, 1])
            ref_lon = float(gps_points[-1, 2])
            count = 1
            for i in range(n - 2, -1, -1):
                if (float(gps_points[i, 1]) == ref_lat and
                    ...
                    break
            remaining_after_start = n - start_trim
            if count >= 2:
                end_trim = min(count, max_trim - start_trim)
                # Ensure we don't trim everything
                end_trim = min(end_trim, remaining_after_start - 2)
                end_trim = max(end_trim, 0)

        # Apply trimming
        if start_trim > 0 or end_trim > 0:
            end_idx = n - end_trim if end_trim > 0 else n
            gps_points = gps_points[start_trim:end_idx]

        return gps_points

    def _clean_speed_outliers(self, gps_points, category):
        """
        Detect and REPLACE speed outlier points instead of removing them.

        Professor's key concern:
            "If we remove points in the trajectories, we should be sure
            that is not creating more outliers."

        Solution: REPLACE instead of REMOVE.
            When a GPS point creates an unrealistic speed (e.g., walking
            at 500 km/h due to GPS signal loss), instead of deleting it
            (which creates a gap that could produce new outliers), we
            replace its coordinates with linearly interpolated values
            from the nearest valid neighbors.

        Algorithm:
            1. Compute real speed between consecutive points (Haversine)
            2. Flag points where speed > max_speed for that transport mode
            3. For each flagged point: replace lat/lon with interpolation
               from nearest valid point before and after it
            4. This preserves trajectory density and prevents cascading

        Args:
            gps_points: Array [timestamp, lat, lon, speed]
            category: Transport mode category (WALKING, BIKE, CAR, etc.)

        Returns:
            Cleaned gps_points array (same length, no points removed)
        """
        if len(gps_points) < 3:
            return gps_points

        max_speed = self.max_speeds.get(category, 200)
        n = len(gps_points)

        # --- Step 1: Flag outlier points ---
        is_outlier = np.zeros(n, dtype=bool)
        # First and last points are never flagged (they are anchors)

        for i in range(1, n):
            dist = haversine_distance(
                float(gps_points[i - 1, 1]), float(gps_points[i - 1, 2]),
                float(gps_points[i, 1]), float(gps_points[i, 2])
            )
            time_diff = (
                gps_points[i, 0] - gps_points[i - 1, 0]
            ).total_seconds() / 3600.0

            if time_diff > 0:
                computed_speed = dist / time_diff
                if computed_speed > max_speed:
                    # Don't flag if it's the last point and the previous
                    # is also flagged — likely the previous was the real
                    # outlier, not this one
                    is_outlier[i] = True

        # First point is always valid (anchor)
        is_outlier[0] = False
        ...
        # If nearly all points are outliers, data is too corrupt — skip cleaning
        if n_outliers > n * 0.8:
            logger.warning(
                f"Too many outliers ({n_outliers}/{n}) for category "
                f"{category}, skipping speed cleaning"
            )
            return gps_points

        # --- Step 2: Replace outlier coordinates ---
        gps_points = gps_points.copy()  # Don't modify original
        ...
        for i in range(n):
            ...
            # Find nearest valid point BEFORE i
            prev_valid = valid_indices[valid_indices < i]
            # Find nearest valid point AFTER i
            next_valid = valid_indices[valid_indices > i]

            if len(prev_valid) > 0 and len(next_valid) > 0:
                p = prev_valid[-1]  # Closest valid before
                q = next_valid[0]   # Closest valid after
                ...
                total_dt = (t_q - t_p).total_seconds()
                if total_dt > 0:
                    ratio = (t_i - t_p).total_seconds() / total_dt
                else:
                    ratio = 0.5

                # Interpolate lat and lon
                gps_points[i, 1] = float(gps_points[p, 1]) + \
                    ratio * (float(gps_points[q, 1]) - float(gps_points[p, 1]))
                gps_points[i, 2] = float(gps_points[p, 2]) + \
                    ratio * (float(gps_points[q, 2]) - float(gps_points[p, 2]))

            elif len(prev_valid) > 0:
                # No valid point after — copy from last valid
                p = prev_valid[-1]
                gps_points[i, 1] = gps_points[p, 1]
                gps_points[i, 2] = gps_points[p, 2]

            elif len(next_valid) > 0:
                # No valid point before — copy from next valid
                q = next_valid[0]
                gps_points[i, 1] = gps_points[q, 1]
                gps_points[i, 2] = gps_points[q, 2]

        logger.debug(
            f"Replaced {n_outliers}/{n} outlier points "
            f"(category={category}, max_speed={max_speed} km/h)"
        )
